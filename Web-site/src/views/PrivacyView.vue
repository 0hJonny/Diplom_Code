...
async function get_articles_count() {
  const { data }: any = await service.get(
    `${urls.articles_count}?language_code=${languageLocales[document.documentElement.getAttribute("lang") || "en"]}&category=privacy`
  );
...
<template>
  <div class="w-full h-full">
    <Header />
    <TitleHeader titleHeader="Privacy" />
  </div>
  <div class="container mx-auto p-4">
    <div class="articles-container">
      ...
      <div class="px-8">
        <ArticlesList :lang="lang" :pagesData="pagesData" category="privacy" />
      </div>
